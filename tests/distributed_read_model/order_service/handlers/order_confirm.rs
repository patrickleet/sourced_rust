//! Reacts to the saga's completed event: transitions the `Order` aggregate and
//! publishes `order.confirmed` (a bitcode snapshot) so the order projector
...
pub const COMMAND: &str = event::COMPLETED;
