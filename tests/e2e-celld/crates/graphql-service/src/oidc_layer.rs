...
use distributed::graphql::{
    graphql_router_with_host, AuthError, IdentityConfig, IdentityMode, IdentityResolver,
...
pub async fn serve_with_oidc_and_host(
    ...
    let engine = service
        .graphql_engine()
        .ok_or_else(|| std::io::Error::other("serve_with_oidc_and_host requires GraphQL"))?;
    let ingress = service.clone();
    let scrape = service.clone();
