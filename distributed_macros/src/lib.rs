...
///   column name.
/// - `#[readmodel(text)]` maps a custom serde value to SQL text. The value must
///   serialize as a JSON string (or as null only when the field is nullable).
...
///
/// Model fields, complete primary keys, relationships, and input paths are
/// resolved through markers generated by `ReadModel` and `GraphqlInput`.
/// Unknown fields, incomplete composite keys, and type mismatches therefore
/// fail at the declaration site.
#[proc_macro]
...
///
/// Defaults are evaluated once by the client runtime before hashing, optimistic
/// effects, and dispatch. Effect declarations then reference `input.field` so
/// the server receives the same generated value the cache used.
#[proc_macro]
...
///
/// The projector value is reused from `GraphqlEngineBuilder::client_projectors`;
/// model keys and optional partitions use the same checked expression DSL as
/// `command_effects!`.
#[proc_macro]
